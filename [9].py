...
-------------------------------------------------
Watermarking:
Hiding a small secret message inside a media file (image, audio, etc.) so you can later prove origin or ownership.

Cryptographic Watermark:
A watermark that is encrypted, so only someone with the correct password can read it.

LSB (Least Significant Bit) Steganography:
Technique of hiding bits of secret data by modifying the least-important bit of each image byte.
Visually invisible in normal images.

Salt:
Random bytes added to key derivation so that even the same password produces different keys each time.

KDF / PBKDF2:
Key-Derivation Function (PBKDF2 here) — transforms a password and salt into a strong encryption key
by repeating hash computations many times.

Fernet:
A high-level symmetric encryption method (from the cryptography library) that provides authenticated encryption
— meaning it encrypts and also ensures data integrity.

Payload:
The serialized bytes actually embedded into the image (contains magic header + salt + length + ciphertext).

🗣 Short Oral Line:
“This tool encrypts a secret message with a password, packages it with a header,
then hides its bits in the least-significant bits of an RGB image.
Extraction reads those bits, verifies the header, and decrypts the message using the same password.”


🔍 High-Level Overview
-------------------------------------------------
The program embeds an encrypted watermark into the LSBs of an image and can later extract and decrypt it.
It supports both a Command Line Interface (CLI) and a Tkinter-based Graphical User Interface (GUI).
...
-------------------------------------------------
- PIL.Image → For reading, writing, and accessing raw image bytes.
- cryptography (Fernet, PBKDF2HMAC, hashes) → For key derivation and authenticated encryption.
- tkinter → For GUI (optional).
- argparse, pathlib, base64, os, sys → For CLI parsing, file paths, encoding, and utilities.

🗣 Oral Line:
“We import Pillow for image processing and Cryptography for encryption; Tkinter is optional for GUI use.”


2️⃣ Constants and Header Format
-------------------------------------------------
MAGIC = b"WMK1"          → Fixed 4-byte header identifying watermark presence.
HEADER_SIZE = len(MAGIC) + 16 + 4  → Header includes magic + 16-byte salt + 4-byte ciphertext length.

🗣 Why:
Magic helps locate the watermark,
Salt is needed for deriving the encryption key,
Length tells how many bytes of ciphertext follow.


3️⃣ WatermarkPayload Dataclass
-------------------------------------------------
- Holds salt and ciphertext.
- serialize() → Builds MAGIC || salt || length(4 bytes) || ciphertext.
- deserialize() → Reads and validates these same fields during extraction.

🗣 Oral Line:
“We pack and unpack the watermark with a header and ciphertext, so extraction knows where to stop.”


4️⃣ Watermarker Class — Core Crypto & Stego Logic
-------------------------------------------------
derive_key(password, salt):
- Uses PBKDF2HMAC(SHA256, 390000 iterations) to derive a strong 32-byte key.
- Encodes the key with Base64 for Fernet compatibility.
🗣 “PBKDF2 turns the password and salt into a strong, unique key.”

encrypt(message, password):
- Generates a random 16-byte salt.
- Derives key using password + salt.
- Uses Fernet(key).encrypt(message) → returns ciphertext token (encrypted + integrity-checked).
- Returns WatermarkPayload(salt, token).

🗣 “Message is encrypted with Fernet and a password-derived key; salt ensures unique key per run.”

decrypt(payload, password):
- Re-derives key using the same salt.
- Decrypts ciphertext; raises error if wrong password or data tampered.

🗣 “Only the correct password decrypts successfully; tampered or wrong inputs fail verification.”


5️⃣ Embedding Process
-------------------------------------------------
_embed_bytes(input_path, output_path, data):

- Load image and convert to RGB mode.
- Extract raw bytes using image.tobytes().
- Convert payload into bit_stream (MSB → LSB order per byte).
- Ensure capacity: one secret bit per image byte.
- Replace each image byte’s least significant bit:
     new_byte = (original_byte & 0xFE) | secret_bit
- Save modified image as a new file.

🗣 Key Point:
Only the least significant bit of each color channel changes → visually invisible to human eye.


6️⃣ Extraction Process
-------------------------------------------------
_extract_bytes(input_path):

- Read all image bytes and extract their LSBs.
- Reconstruct header bytes from first HEADER_SIZE * 8 bits.
- Verify MAGIC header.
- Read 4-byte length field → determines ciphertext size.
- Reconstruct the remaining ciphertext bytes.
- Pass reconstructed bytes to WatermarkPayload.deserialize().
- Decrypt message using password-derived key.

🗣 “We first read header bits to know how long the watermark is, then extract and decrypt it.”


7️⃣ Bit Helper Functions
-------------------------------------------------
_bytes_to_bits(data):
→ Converts bytes into bit sequence (MSB to LSB).

_bits_to_bytes(bits):
→ Groups bits into bytes again.

🗣 “Bit order consistency during embedding and extraction ensures correct data reconstruction.”
"""
